...
def draw_detect_res(img, all_boxes):
    ...
    for bi in range(len(all_boxes)):
        ...
        for box in all_boxes[bi]:
            ...
            img_save_path = 'test_{}.jpeg'.format(time.time())
...
def isInsidePolygon(pt, poly):
    ...
    while i < l - 1:
        ...
        print((poly[i][0]<=pt[0] and pt[0] < poly[j][0])or(poly[j][0]<=pt[0] and pt[0]<poly[i][0] ))
        print((pt[1]<(poly[j][1]-poly[i][1]) * (pt[0]-poly[i][0])/(poly[j][0]-poly[i][0])+poly[i][1]))
